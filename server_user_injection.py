...
class APIKeyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # FOR N8N COMMUNITY NODE HEADER ISSUE and Cursor Integration 
...
async def greeting(hint: str) -> str:
    """
    This tool just displays a message and shows user info.
    ...
    print(f"User {user['username']} is using the greeting tool")
    return f"Hey, Lads! This is Felix Kewa and this is my own remote MCP Server! You are authenticated as {user['username']}"


@mcp.tool()
async def add(a: int, b: int) -> int:
    """
    This tool is used to add two numbers and shows user info.
    ...
    request = request_ctx.get()
    user = request.state.user
    print(f"User {user['username']} is using the add tool")
    return a + b
